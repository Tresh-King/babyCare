...
function getFeedingSubscribeRecord(): FeedingSubscribeRecord {
  const record = getStorage<FeedingSubscribeRecord>(
    StorageKeys.FEEDING_SUBSCRIBE_RECORD,
  );
  if (record) {
    return record;
  }
 ...
 */
export async function requestAllFeedingSubscribeMessages(): Promise<
  Map<SubscribeMessageType, "accept" | "reject">
> {
  console.log(
    "[FeedingSubscribe] 开始申请喂养订阅消息:",
    FEEDING_MESSAGE_TYPES,
  );
 ...
 * 记录申请结果
 */
function recordFeedingSubscribeRequest(
  results: Map<SubscribeMessageType, "accept" | "reject">,
) {
...
function showResultToast(
  ...
  const acceptCount = Array.from(results.values()).filter(
    (s) => s === "accept",
  ).length;
...
export function getFeedingSubscribeStats() {
  ...
  const acceptCount = Object.values(status).filter(
    (s) => s.authStatus === "accept",
  ).length;
  const rejectCount = Object.values(status).filter(
    (s) => s.authStatus === "reject",
  ).length;
  const banCount = Object.values(status).filter(
    (s) => s.authStatus === "ban",
  ).length;
  const unknownCount = Object.values(status).filter(
    (s) => s.authStatus === "unknown",
  ).length;
