 ...
 */
async function uploadSubscribeAuth(
...
export async function requestMultipleSubscribeAuth(
  ...
  try {
    ...
    const res = (await uni.requestSubscribeMessage({ tmplIds })) as any;
    ...
    return {
      success: [],
      failed: templateTypes.map((t) => SUBSCRIBE_TEMPLATES[t].templateType),
    };
  }
}
