 ...
 * 授权记录 Map
 */
const authRecords = reactive<Map<SubscribeMessageType, SubscribeAuthRecord>>(
  new Map(),
);
 ...
 * 引导显示记录 Map
 */
const guideRecords = reactive<Map<SubscribeMessageType, SubscribeGuideRecord>>(
  new Map(),
);
 ...
 * 提醒配置 Map
 */
const reminderConfigs = reactive<
  Map<SubscribeMessageType, SubscribeReminderConfig>
>(new Map());
...
function initializeIfNeeded() {
  if (!isInitialized) {
    // 加载授权记录
    const savedAuthRecords = getStorage<
      Record<SubscribeMessageType, SubscribeAuthRecord>
    ...
    // 加载引导记录
    const savedGuideRecords = getStorage<
      Record<SubscribeMessageType, SubscribeGuideRecord>
    ...
    // 加载提醒配置
    const savedReminderConfigs = getStorage<
      Record<SubscribeMessageType, SubscribeReminderConfig>
...
): SubscribeMessageTemplate | undefined {
...
function updateAuthRecord(
  ...
  const rejectCount =
    (existingRecord?.rejectCount || 0) + (authStatus === "reject" ? 1 : 0);
  ...
  const newRecord: SubscribeAuthRecord = {
    ...
    rejectCount,
    acceptCount:
      (existingRecord?.acceptCount || 0) + (authStatus === "accept" ? 1 : 0),
    updateTime: now,
  };
...
export function getReminderConfig(
  type: SubscribeMessageType,
): SubscribeReminderConfig | undefined {
  initializeIfNeeded();
...
export function updateReminderConfig(
  type: SubscribeMessageType,
  config: Partial<SubscribeReminderConfig>,
) {
  initializeIfNeeded();
 ...
 * 获取所有提醒配置
 */
export function getAllReminderConfigs(): Map<
  SubscribeMessageType,
  SubscribeReminderConfig
> {
  initializeIfNeeded();
 ...
 */
async function uploadAuthRecordsToBackend(
  authResults: Map<SubscribeMessageType, "accept" | "reject">,
) {
  try {
    // 构建请求数据
    ...
    if (response.code === 0) {
      ...
      console.error(
        "[Subscribe] Failed to upload auth records:",
        response.message,
      );
    }
  } catch (error) {
