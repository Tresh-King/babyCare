...
export const useAIStore = () => {
  // 状态定义
  const analyses = reactive<Record<string, AIAnalysis>>({}); // 分析记录映射
  ...
  const analyzingIds = reactive<Set<string>>(new Set()); // 正在分析的ID集合
  const currentAnalysis = ref<AIAnalysis | null>(null); // 当前分析
  const backgroundPollingEnabled = ref(true); // 是否启用后台轮询
  const pollingTimers = reactive<Map<string, number>>(new Map()); // 轮询定时器映射
  ...
  const createAnalysis = async (
    ...
    try {
      ...
      const response = await createAIAnalysis({
        baby_id: typeof babyId === "string" ? parseInt(babyId) : babyId,
      ...
      const idStr = analysis.id.toString();
      analyses[idStr] = analysis;
      analyzingIds.add(idStr);
   ...
   * 轮询分析状态（内部方法）
   */
  const pollAnalysisStatusInternal = async (analysisId: number | string) => {
    const idStr = analysisId.toString();
    try {
      const result = await apiPollAnalysisStatus(
        idStr,
        (status, progress, message) => {
          // 更新分析状态
          const a = analyses[idStr];
          if (a) {
            a.status = status as AIAnalysisStatus;
          }
          console.log(
            `分析${idStr}状态更新: ${status}, 进度: ${progress}%, 消息: ${message}`,
          );
        },
      );
      ...
      if (result.result && a) {
        analyses[idStr] = {
          ...
          insights: result.result.insights?.map((insight) =>
  ...
  const getAnalysisResult = async (
    ...
    const idStr = analysisId.toString();
    try {
      const response = await getAIAnalysisResult(idStr);
      const analysisData = response.data;

      // 更新本地缓存
      const a = analyses[idStr];
      if (a) {
        a.status = analysisData.status as AIAnalysisStatus;
        if (analysisData.result) {
          a.result = analysisData.result;
          a.score = analysisData.result.score;
          a.insights = analysisData.result.insights?.map((insight) =>
            JSON.stringify(insight),
          );
          a.alerts = analysisData.result.alerts?.map((alert) =>
    ...
    try {
      ...
      if (analysisData.result) {
        const analysis: AIAnalysis = {
          ...
          insights: analysisData.result.insights?.map((insight) =>
            JSON.stringify(insight),
          ),
          alerts: analysisData.result.alerts?.map((alert) =>
            JSON.stringify(alert),
          ),
          created_at: analysisData.created_at,
          updated_at: analysisData.created_at,
        ...
        analyses[analysis.id.toString()] = analysis;
        return analysis;
      }
   ...
   * 清除分析缓存
   */
  const clearAnalysisCache = (analysisId?: number | string) => {
    if (analysisId) {
      delete analyses[analysisId.toString()];
    } else {
      // 清除所有分析缓存
  ...
  const getLatestAnalysisByType = (
    ...
    // 按创建时间排序，返回最新的
    const sorted = typeAnalyses.sort(
      (a, b) =>
  ...
  const getAnalysisOverview = (babyId: number) => {
    const babyAnalyses = Object.values(analyses).filter(
      (analysis) => analysis.baby_id === babyId,
    );

    const completed = babyAnalyses.filter((a) => a.status === "completed");
    const scoredCount = completed.filter((a) => a.score !== undefined).length;

    return {
      total: babyAnalyses.length,
      completed: completed.length,
      pending: babyAnalyses.filter(
        (a) => a.status === "pending" || a.status === "analyzing",
      ).length,
      failed: babyAnalyses.filter((a) => a.status === "failed").length,
      averageScore:
        scoredCount > 0
          ? completed.reduce((sum, a) => sum + (a.score || 0), 0) / scoredCount
          : 0,
    };
  };
  ...
  const startPolling = (
    ...
    const idStr = analysisId.toString();
    // 如果已经在轮询，不重复启动
    if (pollingTimers.has(idStr)) {
      console.log(`分析${idStr}已在轮询中`);
      return;
    }

    apiPollAnalysisStatus(idStr, onStatusUpdate || (() => {}), 30, 2000)
      .then((result) => {
        console.log("轮询完成:", result);
        pollingTimers.delete(idStr);
      })
      .catch((error) => {
    ...
    // 标记为正在轮询
    pollingTimers.set(idStr, Date.now());
  };

  /**
   * 停止轮询
   */
  const stopPolling = (analysisId: number | string) => {
    const idStr = analysisId.toString();
    pollingTimers.delete(idStr);
    analyzingIds.delete(idStr);
  };
   ...
   * 获取需要关注的事项
   */
  const getAttentionItems = (babyId: number | string) => {
    const idNum = typeof babyId === "string" ? parseInt(babyId) : babyId;
    const babyAnalyses = Object.values(analyses).filter(
      (analysis) =>
        analysis.baby_id === idNum && analysis.status === "completed",
    );
