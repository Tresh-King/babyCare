 ...
 * 获取AI分析结果
 */
export const getAIAnalysisResult = (
 ...
 * 获取最新AI分析结果
 */
export const getLatestAIAnalysis = (
...
export const getAIAnalysisStats = (
  babyId: number | string,
  days: number = 30,
): Promise<ApiResponse<AnalysisStatsResponse>> => {
...
export const generateDailyTips = (
  babyId: number | string,
  date?: string,
): Promise<ApiResponse<DailyTipsResponse>> => {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (statusResult.status === "completed") {
        const resultResponse = await getAIAnalysisResult(analysisId);
...
const getGrowthAnalysisChartData = (data: any) => {
  ...
  const predictions = data.predictions.filter(
    (p: any) => p.prediction_type === "growth",
  );
