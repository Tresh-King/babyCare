<template>
  ...
  >
    <canvas
      ...
      :style="{ width: width + 'rpx', height: width + 'rpx' }"
    />
    <view v-if="showPercent" class="progress-text">
      <text
        class="percent-value"
        :style="{ fontSize: percentTextSize + 'rpx', color: percentTextColor }"
      >
        {{ Math.round(percent) }}
      </text>
      <text
        ...
        :style="{
          fontSize: percentTextSize * 0.5 + 'rpx',
...
watch(
  () => [props.percent, props.strokeColor],
  () => {
    drawCircle();
  },
  { deep: true },
