  ...
  <view>
    ...
    <view class="join-container">
      ...
      <view
        ...
        :style="{ paddingTop: navbarTotalHeight + 'rpx' }"
      >
        <!-- 加载状态 -->
          ...
          <view class="permission-card">
            ...
            <view class="permission-desc">
              <text v-if="role === 'admin'"
                >管理员可管理宝宝信息、邀请/移除协作者</text
              >
              <text v-else-if="role === 'editor'"
                >编辑者可记录和编辑所有数据</text
              >
  ...
  try {
    ...
    if (menuButton) {
      ...
      navbarTotalHeight.value =
        Math.round(statusBarHeight.value * 2) + Math.round(menuButton.top);
    ...
    console.warn(
      "[Join Navbar] Failed to get menu button info, using defaults",
      e,
    );
    // 使用默认值
